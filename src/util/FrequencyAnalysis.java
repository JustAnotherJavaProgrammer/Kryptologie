...
public class FrequencyAnalysis {
	...
	public static CharFreqPair[] analyze(String str) {
		...
		chars.sort(new Comparator<CharFreqPair>() {
			...
			public int compare(CharFreqPair o1, CharFreqPair o2) {
				// Reversed order or o1 and o2 from typical comparison, so that elements with a
				// larger absoluteFrequency appear at the top
	...
	static class CharFreqPair {
		...
		public String toString() {
			return ((int) character) + "\t" + character + "\t" + absoluteFrequency + "\t"
					+ ((double) Math.round(relativeFrequency * 10000) / 100.0) + "%";
		}
	}
}